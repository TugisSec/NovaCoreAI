      ...
      <div className={`${sidebarCollapsed ? 'w-0 overflow-hidden' : 'w-80'} transition-all duration-300 border-r border-border bg-card flex flex-col fixed left-0 top-0 h-full z-40`}>
        ...
        <ScrollArea className="flex-1 p-2">
          <div className="space-y-1 mt-6">
            ...
            {chatSessions.map(session => <div key={session.id} className={`group relative flex items-center gap-2 p-3 rounded-lg cursor-pointer transition-all duration-300 ease-out hover:scale-105 ${session.id === currentSessionId ? 'bg-primary/10 text-primary' : 'hover:bg-muted/50 text-muted-foreground hover:text-foreground'}`} onClick={() => switchToChat(session.id)}>
        ...
        <div className={`flex-1 flex flex-col ${messages.every(msg => msg.content === 'WELCOME_MESSAGE') ? 'h-screen overflow-hidden' : ''}`}>
          ...
          left: sidebarCollapsed ? '0' : '320px'
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="p-4 rounded-full hidden sm:block">
                <img src="/lovable-uploads/bf2320b5-8643-4c03-8f78-b4949293424c.png" alt="Nova Core Logo" className="h-10 w-10" />
              </div>
              <div>
